...
include("includes/header.php");
	...
	if(isset($_POST['mode'])&&($_POST['mode']=='insertupdate_msds'))
	{
		$msdsId = '';
		...
		$GLOBALS['err_msg'] = $msdsObj -> editMSDS($msdsId);
		
	}
	
	if(isset($_REQUEST['msdsId'])&&($_REQUEST['msdsId']!=''))
	{
		$msdsId = $_REQUEST['msdsId'];
		$msds_info = $msdsObj -> getMSDS($msdsId);
		$form_title = "Edit MSDS";		
		$msds_name = stripslashes($msds_info['msds_name']);
		$imgloc =  stripslashes($msds_info['msds_file']);
	}
	else
	{
		$form_title = "Add MSDS";
...
/***************************** START OF VALIDATION *************************************************/
		...
		{
			if(frm.msds_name.value.search(/\S/)==-1)
			{
				alert("Please Enter MSDS Name");
				frm.msds_name.focus();
				return false;
			}						
			frm.mode.value = 'insertupdate_msds';
		}
/***************************** END OF VALIDATION *************************************************/
	</script>
<div class="container">
...
<input type="hidden" name="msdsId" value="<?php echo $msdsId; ?>" />
</div>		 <div class="row">
			<div class="col-lg-6">

              <ul class="padded separate-sections">
				<li class="input">
                 <label>MSDS Name:</label>
                <input name="msds_name" type="text" value="<?php echo $msds_name;?>" maxlength="255" id="msds_name" /></li>                
                <li class="input">
                 <label>MSDS File: </label>                 
                <input type="file" name="msds_file" id="msds_file" /></li>
               </li>                
              </ul>
            </div>
          ...
          <div class="form-actions">
            ...
            <button type="button" class="btn btn-default" onclick="window.location='MSDS.php';">Cancel</button>
          </div>
         </form>
      </div>
    </div>  
